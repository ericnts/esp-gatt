...
//static const char remote_device_name[] = "gattc";
static bool connect    = false;
static bool get_server = false;
static esp_gattc_char_elem_t *char_elem_result   = NULL;
//static esp_gattc_descr_elem_t *descr_elem_result = NULL;
static esp_ble_gap_cb_param_t scan_rst;
//static uint16_t alarm_conn_id = 0;
//static uint16_t alarm_srv_start_handle = 0;
//static uint16_t alarm_srv_end_handle = 0;
//static uint16_t alarm_gattc_if = 0xff;

static const char target_adv_dat[] = {0x02, 0x01, 0x06, 0x1b, 0xff, 0xff, 0xff, 0x4e, 0x42};
/* FreeRTOS event group to signal when we are connected & ready to make a request */
//static EventGroupHandle_t gattc_event_group;

//const int ALARM_CONNECTED_BIT = BIT0;
//const int ALARM_AUTH_BIT = BIT1;
//const int ALARM_CLEAR_BIT = BIT2;
//const int ALARM_CONNECTED_BIT = BIT3;
...
//static esp_bt_uuid_t devid_filter_char_uuid = {
//    .len = ESP_UUID_LEN_16,
//    .uuid = {.uuid16 = REMOTE_DEVID_CHAR_UUID,},
//};

static esp_bt_uuid_t state_filter_char_uuid = {
    .len = ESP_UUID_LEN_16,
    .uuid = {.uuid16 = REMOTE_DEV_STATE_CHAR_UUID,},
};
...
static uint32_t gattc_ticks = 0, gattc_max = 0;
static void esp_gap_cb(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
...
void gattc_task(void *pvParameters)
{
    while(1){
        if((connect == true) && (gatStatus & FLAG_ALARM_AUTHED)){
            if(gatStatus & FLAG_CMD_DISCONNECT_REQ){
